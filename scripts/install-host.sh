...
cmd_check() {
  ...
  if command -v claude >/dev/null 2>&1; then
    ...
    # Quick auth check so we surface logged-out state at install time, not on a
    # consultant's first chat attempt. Tolerates missing 'auth status' on older
    # versions by ignoring the failure.
    local auth_state
    auth_state="$(claude auth status 2>/dev/null | grep -oE '"loggedIn":[^,}]+' | head -1)"
    case "$auth_state" in
      *true*)  green "    signed in (shared with Claude Desktop via macOS Keychain)" ;;
      *false*) yellow "    NOT signed in — run 'claude' once on this Mac, or sign in to Claude Desktop." ;;
      *)       : ;;  # auth status unavailable; not necessarily a problem
    esac
  else
    yellow "  claude CLI: not on PATH. The installer will run 'npm install -g @anthropic-ai/claude-code'."
    if ! command -v npm >/dev/null 2>&1; then
      red "    npm is missing — install Node first: brew install node"
    fi
  fi
...
ensure_claude_cli() {
  # The Claude Code CLI needs to be a *standalone* install (npm or native
  # installer). Claude Desktop's bundled Claude Code binary IS usable, but only
  # when launched through Claude Desktop's helper chain — invoking it directly
  # from a headless ttyd session has no auth context and prompts the user to
  # sign in, which is unacceptable in production.
  #
  # The standalone npm install reads Anthropic auth tokens from the macOS
  # Keychain, so if Claude Desktop is already signed in, the CLI inherits that
  # session — no separate sign-in needed.
  if command -v claude >/dev/null 2>&1; then
    return 0
  fi
  # Try auto-installing if npm is present.
  if command -v npm >/dev/null 2>&1; then
    hdr "Installing Claude Code via npm (one-time)"
    if npm install -g @anthropic-ai/claude-code 2>&1 | tail -5; then
      green "  installed: $(command -v claude || true)"
      return 0
    fi
    yellow "  npm install failed. Try manually: npm install -g @anthropic-ai/claude-code"
    return 1
  fi
  red "  The 'claude' CLI is not installed and npm is missing. To proceed:"
  red "     brew install node"
  red "     npm install -g @anthropic-ai/claude-code"
  red "  Then re-run this installer. If you already use Claude Desktop, no"
  red "  separate sign-in is needed — the CLI shares auth via the Keychain."
  return 1
}

remove_legacy_desktop_wrapper() {
  # Earlier versions of this installer wrote a ~/.local/bin/claude wrapper that
  # forwarded to Claude Desktop's bundled binary. That bundled binary needs
  # Claude Desktop's helper chain to authenticate, so calling it from a ttyd
  # session always prompts for sign-in. Remove the wrapper if it's still there
  # so PATH resolves to the npm-installed CLI instead.
  local wrapper="$HOME/.local/bin/claude"
  if [[ -f "$wrapper" ]] && head -3 "$wrapper" 2>/dev/null | grep -q "install-host.sh"; then
    rm -f "$wrapper"
    yellow "  removed legacy bundled-Desktop wrapper at $wrapper"
  fi
}
