...
def _chat_ui_context(consultant_slug: str, user_first_name: str = "") -> str:
    ...
    return (
        ...
        "filename you mention with one of these document extensions: "
        ".docx, .pdf, .csv, .txt, .md. Image filenames (.jpg/.jpeg/.png/"
        ".webp) do NOT auto-chip — they're treated as session inputs "
        "(VaultRE photos, floor plans), so never tell the user to 'tap "
        "to download' a photo you only mentioned in passing. Other "
        ...
        "KEEP DELIVERABLES LEAN — the download chip pulls the WHOLE file "
        "over the tunnel in one go, and large files frequently fail to "
        "transfer (the user sees 'Failed to fetch'). Target well under "
        "~5MB. When building a PDF or anything that embeds photos, "
        "downsample/compress the images first (e.g. cap the longest edge "
        "around 1600px at JPEG quality ~75) instead of embedding "
        "full-resolution originals — a photo-heavy PDF at full res "
        "balloons past 10MB and won't download. If a file still comes out "
        "large, rebuild it lighter BEFORE handing it over; don't offer a "
        "download you expect to choke.\n"
        ""
        ...
        ""
        "PHASE 3 WRITING REFERENCES — before writing the body, anchor on "
        "this consultant's strongest past listings. ALWAYS start with the "
        "plain call `./scripts/references.sh` (no arguments): it returns "
        "the most recent up-voted references for THIS consultant — the "
        "signed-in user's own up-voted listings plus any an admin promoted "
        "team-wide. That recency call is the reliable default; use it "
        "every time. OPTIONALLY, to bias the pick toward a closer setting, "
        "append a FEW setting words (not a sentence): "
        "`./scripts/references.sh \"highland lake fishing\"`. That only "
        "RE-RANKS the same pool by how well the setting matches and falls "
        "back to the recent ones when nothing is close — it never comes "
        "back empty when references exist. Do NOT depend on an exact match "
        "and do NOT pass a long phrase expecting it to filter. Scope is "
        "automatic and server-enforced: only this consultant, this user's "
        "own listings, and the team-public set — never another user's "
        "listings or another consultant's. Study the results for voice, "
        "rhythm and structure; NEVER copy sentences or property facts "
        "across — each listing describes its own property truthfully. An "
        "empty result means none are saved yet: just write from the brand "
        "guide.\n"
        ""
        ...
        "After the Social Media Caption, STOP. Do NOT append a 'Tap "
        "\"Save as listing\" below' line or any similar UI instruction — "
        "the chat renders that button automatically from the "
        "Address/Headline/## Listing shape, and any such line would be "
        "saved verbatim into the listing body as noise. "
        "Do NOT generate a .docx. Only generate the .docx if "
